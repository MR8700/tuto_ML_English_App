...
export default function Home() {
  ...
  const onScenario = (name: 'good' | 'bad' | 'unpredictable') => {
    ...
    setFeedback(`Scenario loaded: ${name}`);
    setRunning(false);
    setDayIndex(0);
  };
  ...
  const onImportExcel = async (file: File) => {
    ...
    try {
      ...
      setFeedback(`Excel imported: ${rows.length} rows. Simulation now uses uploaded rainfall series.`);
      setRunning(false);
      setDayIndex(0);
    } catch (error) {
  ...
  const onClearImported = () => {
    ...
    setFeedback('Imported dataset cleared. Simulation switched back to generated rainfall.');
  };
      ...
      <div className="mx-auto max-w-7xl">
        <header className="mb-6 rounded-3xl border border-zinc-200 bg-white/85 p-6 shadow-xl backdrop-blur">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <h1 className="text-2xl font-black tracking-tight sm:text-3xl">Rainfall Impact and Crop Productivity Simulator</h1>
              <p className="mt-1 text-sm text-zinc-700 sm:text-base">
                Burkina Faso context: irregular rainfall can quickly shift crops from healthy growth to drought risk.
              </p>
            </div>
            <button
          ...
          <div className="mt-3 text-sm text-zinc-700">
            <span className="font-semibold">Local climate:</span> Rainy season June-September, dry season October-May.
          </div>
        </header>
